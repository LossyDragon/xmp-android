...
#pragma ide diagnostic ignored "-Wunused-parameter"
#pragma ide diagnostic ignored "-Wimplicit-function-declaration"
...
{
	return (jboolean) (stop_audio() == 0 ? JNI_TRUE : JNI_FALSE);
}

JNIEXPORT jboolean JNICALL
Java_org_helllabs_android_xmp_Xmp_restartAudio(JNIEnv *env, jobject obj)
{
	return (jboolean) (restart_audio() == 0 ? JNI_TRUE : JNI_FALSE);
}

JNIEXPORT jboolean JNICALL
Java_org_helllabs_android_xmp_Xmp_hasFreeBuffer(JNIEnv *env, jobject obj)
{
	return (jboolean) (has_free_buffer() ? JNI_TRUE : JNI_FALSE);
}
...
Java_org_helllabs_android_xmp_Xmp_getChannelData(JNIEnv *env, jobject obj, jintArray vol, jintArray finalvol, jintArray pan, jintArray ins, jintArray key, jintArray period)
{
	...
	for (i = 0; i < chn; i++) {
                struct xmp_channel_info *ci = (struct xmp_channel_info *) &fi[_before].channel_info[i];
...
Java_org_helllabs_android_xmp_Xmp_getPatternRow(JNIEnv *env, jobject obj, jint pat, jint row, jbyteArray rowNotes, jbyteArray rowInstruments)
{
	...
	(*env)->SetByteArrayRegion(env, rowNotes, 0, chn, (const jbyte *) row_note);
	(*env)->SetByteArrayRegion(env, rowInstruments, 0, chn, (const jbyte *) row_ins);
}
...
Java_org_helllabs_android_xmp_Xmp_getSampleData(JNIEnv *env, jobject obj, jboolean trigger, jint ins, jint key, jint period, jint chn, jint width, jbyteArray buffer)
{
	...
	if (xxs->flg & XMP_SAMPLE_16BIT) {
		/* transient */
		for (i = 0; i < limit; i++) {
			_buffer[i] = (char) (((short *) xxs->data)[pos >> 5] >> 8);
			pos += step;
		}
	...
	(*env)->SetByteArrayRegion(env, buffer, 0, width, (const jbyte *) _buffer);
