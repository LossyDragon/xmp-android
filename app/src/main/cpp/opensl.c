...
static int opensl_open(int sr, int num)
{
	...
	case SAMPLE_RATE_8:
		...
		break;
	case SAMPLE_RATE_2205:
		rate = SL_SAMPLINGRATE_22_05;
		break;
	case SAMPLE_RATE_441:
		rate = SL_SAMPLINGRATE_44_1;
		break;
	case SAMPLE_RATE_480:
	...
	SLDataLocator_AndroidSimpleBufferQueue loc_bufq = {
			SL_DATALOCATOR_ANDROIDSIMPLEBUFFERQUEUE, (SLuint32) num
	};
...
int open_audio(int rate, int latency)
{
	...
	buffer_size = rate * 2 * 2 * BUFFER_TIME / bufferDiv;

	if (buffer_num < 3)
		buffer_num = 3;

	buffer = malloc((size_t) (buffer_size * buffer_num));
	if (buffer == NULL)
		return -1;
...
int fill_buffer(int looped)
{
	...
	lock();
	if (buffer_queue != NULL) {
		(*buffer_queue)->Enqueue(buffer_queue, b, (SLuint32) buffer_size);
	}
	unlock();
